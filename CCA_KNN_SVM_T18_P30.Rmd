...
output:
  html_document:
    df_print: paged
subtitle: </br>  </br> Dataset Integration and Identity Prediction - Postnatal Day 30 </br>  </br> 
---
...
Seurat object with P30 in house dataset containing Htr3a-expressing cortical cells.
...
## STEP 1 </br> </br> CCA Tasic18 with P30 and bicor implementation for dimension selection

Use Seurat version 2 Canonical Correlation Analysis Integration pipeline to project in the same embedding space T18 and P30 cells.
...
P30 <- NormalizeData(object = P30) ; P30 <- ScaleData(object = P30) ; P30 <- FindVariableGenes(object = P30, do.plot = FALSE)
...
hvg.P30<- rownames(x = head(x = P30@hvg.info, n = 2000)) ; hvg.Tasic18 <- rownames(x = head(x = Tasic18@hvg.info, n = 2000))
hvg.union <- union(x = hvg.P30, y = hvg.Tasic18)

# annotate dataset
P30@meta.data[, "protocol"] <- "NkxP30" ; Tasic18@meta.data[, "protocol"] <- "Tasic18"

# perform CCA integration
CCA_merged <- RunCCA(object = P30, object2 = Tasic18, genes.use = hvg.union)
...
CCA_merged <- RunTSNE(object = CCA_merged, reduction.use = "cca.aligned", dims.use = cca.dims, do.fast = TRUE,dim.embed = 2,perplexity=30)
...
# Filter CCA_merged to keep and plot only P30 double positive cells and tasic cells by protocol and cluster
Tand_dp30 <-  SubsetData(CCA_merged,cells.use = CCA_merged@meta.data$Tom_cells==T | CCA_merged@meta.data$protocol %in% c("Tasic18"))
table(Tand_dp30@meta.data$Tom_cells)
TSNEPlot(object = Tand_dp30 ,group.by="protocol",do.return = TRUE, pt.size = 1,do.label=T)
TSNEPlot(object = Tand_dp30 ,group.by="cluster",do.return = TRUE, pt.size = 1,do.label=T,colors.use =rainbow(29))

```

## STEP 2 </br> </br> KNN prediction of the identity of P30 cells based on T18 identity annotation (nearest neighbors classifier on tSNE coordinates)

```{r KNN, include=TRUE, eval=FALSE}

# Split data in T18 and P30 cells
CCA_merged_P30 <- SubsetData(CCA_merged,cells.use = CCA_merged@meta.data$protocol=="NkxP30")
...
# Knn trained on tasic and tested on P30 to predict P30 identity
set.seed(123)
CCA_merged_P30@meta.data$knn.pred <- local({
  x.trn <- GetCellEmbeddings(CCA_merged_T18,"tsne")
  x.tst <- GetCellEmbeddings(CCA_merged_P30,"tsne")
...
with(CCA_merged_P30@meta.data,table(knn.pred,Tom_cells))

# KNN prediction - plot results
dp_P30 <- SubsetData(CCA_merged_P30, cells.use= CCA_merged_P30@meta.data$Tom_cells=="TRUE" & CCA_merged_P30@meta.data$protocol=="NkxP30")
xy <- expand.grid(x = seq(-40,40,length.out=100), y = seq(-45,45,length.out=100))
...
with(dp_P30@meta.data,table(knn.pred,Tom_cells))
...
ggplot(xy) + geom_tile(aes(fill=Y,x=x,y=y)) + 
  geom_point(aes(x=tSNE_1,y=tSNE_2,colour=as.factor(CCA_merged_P30@meta.data$Tom_cells)),data=as.data.frame(GetCellEmbeddings(dp_P30,"tsne")))+
...
## STEP 3 </br> </br> SVM prediction of the identity of P30 cells based on T18 identity annotation (support vector machines classifier on gene expression)
...
# Predict P30 cells from tasic bag
f <- T18.bag.predict(bag,t(CCA_merged_P30@scale.data))
R <- apply(-f,2:3,rank)
R <- apply(R,1:2,mean)
...
CCA_merged_P30@meta.data$bag.pred <- colnames(bag.pred)[apply(bag.pred,1,which.min)]
heatmap <- heatmap3::heatmap3(
  bag.pred,scale="none",labRow=NA,
  Rowv=NA,Colv=as.dendrogram(hc),
  RowSideColors = cbind(bagNGC=ifelse(CCA_merged_P30@meta.data$bag.pred %in% c("Lamp5 Plch2 Dock5","Lamp5 Lsp1"),"black","white"),
                        knnNGC=ifelse(CCA_merged_P30@meta.data$knn.pred$knn.pred=="Lamp5 Plch2 Dock5","black","white"),
                        Tom=ifelse(CCA_merged_P30@meta.data$Tom_cells,"red","white"))
)

```

## STEP 4 </br> </br> Cross-Compare KNN and SVM results to annotate P30 cells consensus identity

```{r consensus, include=TRUE, eval=FALSE}

minbag <- colnames(R)[apply(R,1,which.min)]
knnvsbag <- data.frame(minbag,knn=CCA_merged_P30@meta.data$knn.pred)
knnvsbag$equalpred <- ifelse(as.character(knnvsbag$minbag)==as.character(knnvsbag$knn),TRUE,FALSE)
knnvsbag$cell_names <- CCA_merged_P30@cell.names
table(knnvsbag$equalpred,CCA_merged_P30@meta.data$Tom_cells)

CCA_merged_P30@meta.data$IDconsensus <- ifelse(knnvsbag$equalpred==TRUE,CCA_merged_P30@meta.data$knn.pred,"inconclusive")
...
CCA_KNN_SVM_T18_P30 <- rbind(
  ...
  data.frame(cellnames=CCA_merged_P30@cell.names,dataset="P30",
             tSNE_1=GetCellEmbeddings(CCA_merged_P30,"tsne")[,"tSNE_1"],
             tSNE_2=GetCellEmbeddings(CCA_merged_P30,"tsne")[,"tSNE_2"],
             KNNpred=CCA_merged_P30@meta.data$knn.pred,SVMpred=CCA_merged_P30@meta.data$bag.pred,
             IDconsensus=CCA_merged_P30@meta.data$IDconsensus,Tom_cells=CCA_merged_P30@meta.data$Tom_cells)
  
)

#write.csv(CCA_KNN_SVM_T18_P30,"out/CCA_KNN_SVM_T18_P30.csv",row.names=FALSE)  
```
